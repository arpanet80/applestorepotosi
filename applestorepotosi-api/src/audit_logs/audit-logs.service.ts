...
export class AuditLogsService {
  ...
  async update(id: string, updateAuditLogDto: UpdateAuditLogDto): Promise<AuditLogDocument> {
    ...
    const criticalFields = ['userId', 'collection', 'action', 'documentId', 'before', 'after', 'timestamp'];
   ...
   */
  async findByCollection(collection: AuditCollection): Promise<AuditLogDocument[]> {
    const auditLogs = await this.auditLogModel
      .find({ collection })
      .populate('userId', 'profile.firstName profile.lastName email')
      .sort({ timestamp: -1 })
   ...
   */
  async findByDocument(collection: AuditCollection, documentId: string): Promise<AuditLogDocument[]> {
  ...
  async getStats(query: AuditStatsQueryDto): Promise<{
    ...
    const { startDate, endDate, collection, groupBy = 'day' } = query;
    ...
    if (collection) {
      dateFilter.collection = collection;
    }
  ...
  async logUpdate(
    ...
    return this.logAction(
      ...
      before,
      after,
      notes || `Actualización de ${collection}`
    );
  }
  ...
  async logDelete(
    ...
    return this.logAction(
      ...
      before,
      undefined,
      notes || `Eliminación de ${collection}`
    );
  }
}