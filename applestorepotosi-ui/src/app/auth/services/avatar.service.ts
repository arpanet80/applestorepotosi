...
export class AvatarService {
  ...
  // 1. Si tiene photoURL de Firebase (Google), usar esa
  if (user?.photoURL) {
    return user.photoURL;
  }
  
  // 2. Si tiene avatar en el perfil extendido, usar ese
  if (user?.profile?.avatar) {
    return user.profile.avatar;
  }
  
  // 3. Generar avatar con iniciales del displayName, profile.name o email
