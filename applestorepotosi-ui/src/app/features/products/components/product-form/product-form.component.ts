...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, ImageUploadComponent, ObjectUrlPipe, NgIf  ],
...
export class ProductFormComponent implements OnInit, OnDestroy {
  ...
  */
  ...
  ngOnInit() {
    ...
    if (!this.isEditMode) this.setAutoSku();
  }

  private setAutoSku(): void {
    // prefijo + timestamp + 4 dígitos aleatorios
    const base = 'APL';
    const ts   = Date.now().toString(36).toUpperCase();
    const rnd  = Math.floor(Math.random() * 9000 + 1000);
    const sku  = `${base}-${ts}-${rnd}`;
