...
export class BrandFormComponent implements OnInit, OnDestroy {
  ...
  preview: string | null = null;                // para pre-visualización local
  ...
  startReplace(): void {
  // Solo cambia el flag para mostrar el upload
  this.isReplacing = true;
  this.newLogoFile = null;   // reset por si acaso
  this.preview = null;       // reset preview
}

// onLogoSelected(file: File): void {
//   this.newLogoFile = file;
//   this.preview = URL.createObjectURL(file);
//   // (opcional) limpiar URL externa
//   this.brandForm.patchValue({ logoUrl: '' });
// }

onLogoCancelled(): void {
  // El usuario canceló la selección: volver al estado inicial
  this.newLogoFile = null;
  this.preview = null;
  this.isReplacing = false;
  // Si queremos también limpiar el input file
